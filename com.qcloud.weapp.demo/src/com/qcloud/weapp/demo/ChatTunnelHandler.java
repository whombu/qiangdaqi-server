...
public class ChatTunnelHandler implements TunnelHandler {
	//private static final Logger logger = Logger.getLogger(ChatTunnelHandler.class);
	/**
	...
	public void onTunnelRequest(Tunnel tunnel, UserInfo userInfo) {
		...
		//logger.info(String.format("Tunnel Connected: %s", tunnel.getTunnelId()));
	}
	...
	public void onTunnelConnect(Tunnel tunnel) {
		if (userMap.containsKey(tunnel.getTunnelId())) {
			...
			//logger.info("broadcast connect:" + peopleMessage);
	...
	/**
	 * 实现 OnTunnelMessage 方法 客户端推送消息到 WebSocket 信道服务器上后，会调用该方法，此时可以处理信道的消息。
	...
	public void onTunnelMessage(Tunnel tunnel, TunnelMessage message) {
		if (message.getType().equals("speak") && userMap.containsKey(tunnel.getTunnelId())) {
			...
			try {
				...
				//logger.info("word:" + messageContent.getString("word"));
				//logger.info("who:" + new JSONObject(userMap.get(tunnel.getTunnelId())));
			} catch (JSONException e) {
	...
	/**
	 * 实现 OnTunnelClose 方法 客户端关闭 WebSocket
	 * 信道或者被信道服务器判断为已断开后，会调用该方法，此时可以进行清理及通知操作
	 */
	@Override
